...
pub fn apply_update(download_url: &str) -> Result<(), String> {
    ...
    let mut file = File::create(&new_exe).map_err(|e| format!("Failed to create temp file: {}", e))?;
    std::io::copy(&mut reader, &mut file).map_err(|e| format!("Download transfer failed: {}", e))?;
    drop(file);
